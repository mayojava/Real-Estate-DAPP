...
contract Ownable {
    ...
    event OwnershipTransfered(address from, address newOwner);
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        ...
        require(newOwner != address(0), "You cannot transfer to 0 address");
        
        address _oldOwner = _owner;
        _owner = newOwner;

        emit OwnershipTransfered(_oldOwner, _owner);
    }
}
...
contract ERC721 is Pausable, ERC165 {
    ...
    function balanceOf(address owner) public view returns (uint256) {
        ...
        Counters.Counter storage count = _ownedTokensCount[owner];
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(owner != to, "To address is already the owner of the token");

        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(owner == msg.sender || isApprovedForAll(owner, msg.sender), "Caller is not the owner of token and token is not approved for all");
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        require(!_exists(tokenId), "This token - tokenId - already exists");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        Counters.Counter storage fromTokens = _ownedTokensCount[from];
        Counters.Counter storage toTokens = _ownedTokensCount[to];

        fromTokens.decrement();
        toTokens.increment();
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function getBaseTokenUri() public view returns(string memory) { return _baseTokenURI; }
    ...
    function setTokenUri(uint256 tokenId, string memory tokenUri) internal {
        require(_exists(tokenId));
        string memory finaluri = strConcat(_baseTokenURI, tokenUri, uint2str(tokenId));
        _tokenURIs[tokenId] = finaluri;
    }
}
...
contract CustomERC721Token is ERC721Metadata("CustomERC721Token", "CusERCTok", "https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/") {
